pipeline {
    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
                container('maven') {
                    ...
                    sh "mvn clean package -P${params.PROFILE} -Dmaven.test.skip=true"
                }
            }
        }
        ...
        stage('Docker image build') {
            steps {
                container('docker') {
                    withEnv(["PROFILE=${params.PROFILE}"]) {
                        ...
                        sh 'docker build --build-arg ACTIVE_PROFILE=${PROFILE} -t ${DOCKER_IMAGE}:${BUILD_NUMBER} .'
                    }
                }
            }
        }
